...
static void lvgl_port_flush_io_ready_callback(void *user_ctx);
#if CONFIG_IDF_TARGET_ESP32S3 && ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
...
{
    lvgl_port_lock(0);
    //创建lvgl display对象，此对象被lvgl调用，保存了lcd fulsh，屏幕参数，分配的缓存等信息，实现对显示驱动的调用
    //显示驱在esp_board_manager组件实现。
...
#if LVGL_PORT_HANDLE_FLUSH_READY
        //esp_lcd_panel_io_register_event_callbacks()已在esp_board_manager组件中被配置，
        //我们在这里把esp_lvgl_port组件的回调函数赋值给驱动显示设备的成员，即可现对esp_lvgl_port组件的回调函数调用
        //回调函数最终是调用了lvgl的lv_disp_flush_ready(),通知上一次显示操作完成，lvgl可调用flush函数再次刷新
        dev_display_lcd_handles_t *disp_handle;
        esp_board_manager_get_device_handle("display_lcd", (void **)&disp_handle);
        disp_handle->transfer_done_cb = lvgl_port_flush_io_ready_callback;
        disp_handle->transfer_done_user_data = disp;
...
#error please disable LV_COLOR_DEPTH_1 for using monochromatic screen
    ...
    lv_display_set_flush_cb(disp, lvgl_port_flush_callback); //设置刷屏回调函数
...
static void lvgl_port_flush_callback(lv_display_t *drv, const lv_area_t *area, uint8_t *color_map)
{
    ...
    //以下情况不需要显示驱动通知显示操作完成，调用上面函数显示后直接通知lvgl可进和下次刷屏。
    if (disp_ctx->disp_type == LVGL_PORT_DISP_TYPE_RGB || (disp_ctx->disp_type == LVGL_PORT_DISP_TYPE_DSI && (disp_ctx->flags.direct_mode || disp_ctx->flags.full_refresh))) {
