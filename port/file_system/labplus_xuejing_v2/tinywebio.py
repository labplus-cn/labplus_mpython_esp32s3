    ...
    def write(self, raw_tag, raw_value):
        ...
        try:
            if tag.startswith('rgb'):
                ...
                self.gui.set_bg_color(0x000000)
                if method == 'show':
                    content, x, y = vdata.split(',')
                    self.gui.fill(type=0)
                    self.gui.draw_label(text=str(content), x=int(x), y=int(y), width=290, color=0xffffff, wrap=False)
                elif method == 'fill':
                    self.gui.fill(type=0)
                else:
                    pass
                ...
                pin = int(tag[5:])
                param = value.strip()
                if pin not in list(servo_list.keys()):
                    servo_list[pin]=Servo(pin, min_us=500, max_us=2500, actuation_range=180)
                servo_list[pin].write_angle(int(param))
            elif tag.startswith('MPythonPin'):
                pname, pmode, pnum = tag.split('_')
                pfun = getattr(mpython, pname)
                n = int(pnum)
...
class Server:
    ...
    def start(self, gui = None, port=8888):
        self.gui = gui
        self._start_server(port, self.connect_client)

    def start_foreground(self, gui = None, port=8888):
        self.gui = gui
