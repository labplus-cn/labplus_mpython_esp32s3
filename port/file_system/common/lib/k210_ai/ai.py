...
class KPU_MODEL(object):
    """ 使用自定义模型 """
    def __init__(self, uart, choice=1, width=320, height=240, komodel_path=''):
        self.uart = uart
        self.choice = choice
        ...
        self.lock = False
        self.send_num = 0
        parameter = str(komodel_path)+'|'+ str(width)+'|'+ str(height)
        AI_Uart_CMD_String(uart=self.uart, cmd=self.CommandList[0], cmd_type=self.CommandList[2], cmd_data=[self.choice,0,0], str_buf=parameter)
        time.sleep(0.5)
...
class KPU_YOLO_MODEL(object):
    """ 使用自定义模型 YOLO"""
    def __init__(self, uart, choice=1, width=128, height=128, komodel_path='', anchors=[]):
        self.uart = uart
        self.choice = choice
        self.CommandList = AI['kpu_model_yolo']
        self.id = None
        self.max_score = 0
        self._task = None
        self.lock = False
        self.send_num = 0
        parameter = str(komodel_path)+'|'+ str(width)+'|'+ str(height)+'|'+ str(anchors)
        AI_Uart_CMD_String(uart=self.uart, cmd=self.CommandList[0], cmd_type=self.CommandList[2], cmd_data=[self.choice,0,0], str_buf=parameter)
        time.sleep(0.5)
