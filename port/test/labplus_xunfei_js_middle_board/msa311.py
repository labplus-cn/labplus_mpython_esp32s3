...
# Interrupt source
INT_SRC_MOTION = const(0x01)
INT_SRC_TAP = const(0x02)
INT_SRC_ORIENT = const(0x04)
INT_SRC_FREEFALL = const(0x08)
INT_SRC_DATA_READY = const(0x10)
...
class MSA311:
    def __init__(self, i2c, int_pin, addr=MSA311_ADDR, g_range=G_RANGE_2G, odr=ODR_125HZ):
        self.i2c = i2c
        self.addr = addr
        ...
        self.int_pin.irq(trigger=Pin.IRQ_RISING, handler=self._interrupt_handler)
    ...
    def config_interrupt(self, od_lvl= 0x02, latch = 0x00):
        self._write_reg(REG_INT_CONFIG, od_lvl)
        self._write_reg(REG_INT_LATCH, latch)
        self._write_reg(REG_INT_MAP0, (INT_ACTIVE_MAP | INT_FREEFALL_MAP | INT_DOUBLE_TAP_MAP | INT_SINGLE_TAP_MAP | INT_ORIENT_MAP))
    ...
    def _init_sensor(self, g_range, odr):
        ...
        self.set_odr(odr)
        self.set_power_mode(0x00)
        self.set_axis_swap_polarity(xy_swap=False, x_pol=0, y_pol=0, z_pol=0)
    ...
    def read_accel(self):
        ...
        return (x * self._acc_scale, y * self._acc_scale, z * self._acc_scale)
    ...
    def _interrupt_handler(self, pin):
        ...
        orient_status = self._read_reg(REG_ORIENT_STATUS)
        ...
        if motion_int_status & INT_SRC_MOTION and self.active_callback:
            self.active_callback()
            
        # Check for tap detection
        if tap_status and self.tap_callback:
            self.tap_callback()
            
        # Check for orientation change
        if orient_status and self.orientation_callback:
    ...
    def configure_orientation_detection(self, int_enable = True, callback=None):     
    ...
    def configure_freefall_detection(self, int_enable = True, threshold=0x30, duration=0x09, callback=None):
    ...
    def configure_active_detection(self, axes=ORIENT_AXIS_ALL, threshold=0x0A, duration=0x00, callback=None):
        # Configure active detection
        self._write_reg(REG_ACTIVE_TH, threshold)
        self._write_reg(REG_ACTIVE_DUR, duration)
        ...
        if axes & FREEFALL_AXIS_X:
            int_set |= INT_ACTIVE_X_EN
        if axes & FREEFALL_AXIS_Y:
            int_set |= INT_ACTIVE_Y_EN
        if axes & FREEFALL_AXIS_Z:
            int_set |= INT_ACTIVE_Z_EN
