...
you can define the functions LP_lodepng_free, LP_lodepng_malloc and LP_lodepng_realloc in your
...
LP_lodepng_crc32 must be defined externally so that it can be linked in.
...
Return value: LP_LodePNG error code (0 means no error).
*/
...
/*Same as LP_lodepng_decode_memory, but always decodes to 32-bit RGBA raw image*/
unsigned LP_lodepng_decode32(unsigned char** out, unsigned* w, unsigned* h,
                          const unsigned char* in, size_t insize);

/*Same as LP_lodepng_decode_memory, but always decodes to 24-bit RGB raw image*/
...
/*Same as LP_lodepng_decode_file, but always decodes to 32-bit RGBA raw image.
...
/*Same as LP_lodepng_decode_file, but always decodes to 24-bit RGB raw image.
...
/*Same as LP_lodepng_encode_memory, but always encodes from 32-bit RGBA raw image.*/
unsigned LP_lodepng_encode32(unsigned char** out, size_t* outsize,
                          const unsigned char* image, unsigned w, unsigned h);

/*Same as LP_lodepng_encode_memory, but always encodes from 24-bit RGB raw image.*/
...
/*Same as LP_lodepng_encode_file, but always encodes from 32-bit RGBA raw image.
...
/*Same as LP_lodepng_encode_file, but always encodes from 24-bit RGB raw image.
...
namespace lodepng {
#ifdef LODEPNG_COMPILE_DECODER
/*Same as LP_lodepng_decode_memory, but decodes to an std::vector. The colortype
is the format to output the pixels to. Default is RGBA 8-bit per channel.*/
unsigned decode(std::vector<unsigned char>& out, unsigned& w, unsigned& h,
                const unsigned char* in, size_t insize,
                LP_LodePNGColorType colortype = LCT_RGBA, unsigned bitdepth = 8);
unsigned decode(std::vector<unsigned char>& out, unsigned& w, unsigned& h,
...
#ifdef LODEPNG_COMPILE_ENCODER
/*Same as LP_lodepng_encode_memory, but encodes to an std::vector. colortype
...
struct LP_LodePNGDecompressSettings {
  /* Check LP_LodePNGDecoderSettings for more ignorable errors such as ignore_crc */
...
struct LP_LodePNGCompressSettings /*deflate = compress*/ {
...
format, and is used both for PNG and raw image data in LP_LodePNG.
*/
typedef struct LP_LodePNGColorMode {
  /*header (IHDR)*/
  LP_LodePNGColorType colortype; /*color type, see PNG standard or documentation further in this header file*/
  ...
  This field may not be allocated directly, use LP_lodepng_color_mode_init first,
  then LP_lodepng_palette_add per color to correctly initialize it (to ensure size
  of exactly 1024 bytes).

  The alpha channels must be set as well, set them to 255 for opaque images.

  When decoding, with the default settings you can ignore this palette, since
  LP_LodePNG already fills the palette colors in the pixels of the raw RGBA output,
  ...
  */
  ...
  When decoding, by default you can ignore this information, since LP_LodePNG sets
...
/* Makes a temporary LP_LodePNGColorMode that does not need cleanup (no palette) */
...
typedef struct LP_LodePNGInfo {
  ...
  LP_LodePNGColorMode color;     /*color type and bits, palette and transparency of the PNG file*/
  ...
  */
  ...
  correctly and use LP_lodepng_add_text and LP_lodepng_clear_text.
  ...
  */
  ...
  correctly and use LP_lodepng_set_exif and LP_lodepng_clear_exif.
  ...
  */
  ...
  LP_LodePNG does not apply any color conversions on pixels in the encoder or decoder and does not interpret these color
  profile values. It merely passes on the information. If you wish to use color profiles and convert colors, a separate
  color management library should be used. There is also a limited library for this in LP_lodepng_util.h.

  There are 4 types of (sets of) chunks providing color information. If multiple are present, each will be decoded by
  LP_LodePNG, but only one should be handled by the user, with the following order of priority depending on what the user
  supports:
  ...
  */
  ...
  its specification for more details. LP_LodePNG does not parse or use the ICC profile (except its color space header
  ...
  to the user to compute approximate values and set then in the appropriate gama_ and chrm_ fields, LP_LodePNG does not do
  ...
  */
  ...
  Don't allocate this buffer yourself. Use the init/cleanup functions
  correctly and use LP_lodepng_set_icc and LP_lodepng_clear_icc.
  */
  ...
  The meaning of the values are as defined in the specification ITU-T-H.273. LP_LodePNG does not
  ...
  */
  ...
  /*
  unknown chunks: chunks not known by LP_LodePNG, passed on byte for byte.
  ...
  this if you wish to store an ancillary chunk that is not supported by LP_LodePNG (such as sPLT or hIST),
  or any non-standard PNG chunk.

  Do not allocate or traverse this data yourself. Use the chunk traversing functions declared
  later, such as LP_lodepng_chunk_next and LP_lodepng_chunk_append, to read/write this struct.
  */
...
#ifdef LODEPNG_COMPILE_ANCILLARY_CHUNKS
unsigned LP_lodepng_add_text(LP_LodePNGInfo* info, const char* key, const char* str); /*push back both texts at once*/
void LP_lodepng_clear_text(LP_LodePNGInfo* info); /*use this to clear the texts again after you filled them in*/

unsigned LP_lodepng_add_itext(LP_LodePNGInfo* info, const char* key, const char* langtag,
                           const char* transkey, const char* str); /*push back the 4 texts of 1 chunk at once*/
void LP_lodepng_clear_itext(LP_LodePNGInfo* info); /*use this to clear the itexts again after you filled them in*/

/*replaces if exists*/
unsigned LP_lodepng_set_icc(LP_LodePNGInfo* info, const char* name, const unsigned char* profile, unsigned profile_size);
void LP_lodepng_clear_icc(LP_LodePNGInfo* info); /*use this to clear the profile again after you filled it in*/

/*replaces if exists*/
unsigned LP_lodepng_set_exif(LP_LodePNGInfo* info, const unsigned char* exif, unsigned exif_size);
void LP_lodepng_clear_exif(LP_LodePNGInfo* info); /*use this to clear the exif metadata again after you filled it in*/
#endif /*LODEPNG_COMPILE_ANCILLARY_CHUNKS*/

/*
Converts raw buffer from one color type to another color type, based on
LP_LodePNGColorMode structs to describe the input and output color type.
See the reference manual at the end of this header file to see which color conversions are supported.
return value = LP_LodePNG error code (0 if all went ok, an error if the conversion isn't supported)
The out buffer must have size (w * h * bpp + 7) / 8, where bpp is the bits per pixel
of the output color type (LP_lodepng_get_bpp).
For < 8 bpp images, there should not be padding bits at the end of scanlines.
For 16-bit per channel colors, uses big endian format like PNG does.
Return value is LP_LodePNG error code
*/
...
typedef struct LP_LodePNGDecoderSettings {
  LP_LodePNGDecompressSettings zlibsettings; /*in here is the setting to ignore Adler32 checksums*/

  /* Check LP_LodePNGDecompressSettings for more ignorable errors such as ignore_adler32 */
  ...
  /*store all bytes from unknown chunks in the LP_LodePNGInfo (off by default, useful for a png editor)*/
...
/*Get a LP_LodePNGColorStats of the image. The stats must already have been inited.
...
typedef struct LP_LodePNGEncoderSettings {
  LP_LodePNGCompressSettings zlibsettings; /*settings for the zlib encoder, such as window size, ...*/
  ...
  have to cleanup this buffer, LP_LodePNG will never free it. Don't forget that filter_palette_zero
...
#ifdef LODEPNG_COMPILE_ANCILLARY_CHUNKS
  /*add LP_LodePNG identifier and version as a text chunk, for debugging*/
  unsigned add_id;
...
#ifdef LODEPNG_COMPILE_DECODER
  LP_LodePNGDecoderSettings decoder; /*the decoding settings*/
#endif /*LODEPNG_COMPILE_DECODER*/
#ifdef LODEPNG_COMPILE_ENCODER
  LP_LodePNGEncoderSettings encoder; /*the encoding settings*/
#endif /*LODEPNG_COMPILE_ENCODER*/
  LP_LodePNGColorMode info_raw; /*specifies the format in which you would like to get the raw pixel buffer*/
  LP_LodePNGInfo info_png; /*info of the PNG image obtained after decoding*/
  unsigned error;
} LP_LodePNGState;
...
#ifdef LODEPNG_COMPILE_DECODER
/*
Same as LP_lodepng_decode_memory, but uses a LP_LodePNGState to allow custom settings and
...
information is placed in the info_png field of the LP_LodePNGState.
*/
...
/*
Reads one metadata chunk (other than IHDR, which is handled by LP_lodepng_inspect)
of the PNG file and outputs what it read in the state. Returns error code on failure.
Use LP_lodepng_inspect first with a new state, then e.g. LP_lodepng_chunk_find_const
to find the desired chunk type, and if non null use LP_lodepng_inspect_chunk (with
chunk_pointer - start_of_file as pos).
Supports most metadata chunks from the PNG standard (gAMA, bKGD, tEXt, ...).
Ignores unsupported, unknown, non-metadata or IHDR chunks (without error).
Requirements: &in[pos] must point to start of a chunk, must use regular
LP_lodepng_inspect first since format of most other chunks depends on IHDR, and if
...
/*
The LP_lodepng_chunk functions are normally not needed, except to traverse the
unknown chunks stored in the LP_LodePNGInfo struct, or add new ones to it.
...
Input must be at the beginning of a chunk (result of a previous LP_lodepng_chunk_next call,
...
public interface only for tests, it's used internally by LP_lodepng_deflate.
*/
...
#ifdef LODEPNG_COMPILE_CPP
/* The LP_LodePNG C++ wrapper uses std::vectors instead of manually allocated memory buffers. */
namespace lodepng {
#ifdef LODEPNG_COMPILE_PNG
...
[ ] let the C++ wrapper catch exceptions coming from the standard library and return LP_LodePNG error codes
...
LP_LodePNG is a PNG codec according to the Portable Network Graphics (PNG)
...
The most recent version of LP_LodePNG can currently be found at
http://lodev.org/lodepng/

LP_LodePNG works both in C (ISO C90) and C++, with a C++ wrapper that adds
extra functionality.

LP_LodePNG exists out of two files:
-lodepng.h: the header file for both C and C++
-lodepng.c(pp): give it the name lodepng.c or lodepng.cpp (or .cc) depending on your usage

If you want to start using LP_LodePNG right away without reading this doc, get the
examples from the LP_LodePNG website to see how to use it in code, or check the
smaller examples in chapter 13 here.

LP_LodePNG is simple but only supports the basic requirements. To achieve
simplicity, the following design choices were made: There are no dependencies
on any external library. There are functions to decode and encode a PNG with
a single function call, and extended versions of these functions taking a
LP_LodePNGState struct allowing to specify or get more information. By default
the colors of the raw image are always RGB or RGBA, no matter what color type
the PNG file uses. To read and write files, there are simple functions to
convert the files to/from buffers in memory.

This all makes LP_LodePNG suitable for loading textures in games, demos and small
...
*) colorimetric color profile conversions: currently experimentally available in LP_lodepng_util.cpp only,
...
lodepng::State class which is a LP_LodePNGState with constructor and destructor.
...
Even if carefully designed, it's always possible that LP_LodePNG contains possible
exploits. If you discover one, please let me know, and it will be fixed.

When using LP_LodePNG, care has to be taken with the C version of LP_LodePNG, as well
...
LP_lodepng_decode32, and more advanced decoding can be done with the struct
LP_LodePNGState and LP_lodepng_decode. For C++, all decoding can be done with the
various lodepng::decode functions, and lodepng::State can be used for advanced
features.

When using the LP_LodePNGState, it uses the following fields for decoding:
*) LP_LodePNGInfo info_png: it stores extra information about the PNG (the input) in here
*) LP_LodePNGColorMode info_raw: here you can say what color mode of the raw image (the output) you want to get
*) LP_LodePNGDecoderSettings decoder: you can specify a few extra settings for the decoder to use
...
suggested background color, etc... More details about the LP_LodePNGInfo struct are
...
color type information in the LP_LodePNGInfo.
...
LP_lodepng_encode32, and more advanced decoding can be done with the struct
LP_LodePNGState and LP_lodepng_encode. For C++, all encoding can be done with the
...
When using the LP_LodePNGState, it uses the following fields for encoding:
*) LP_LodePNGInfo info_png: here you specify how you want the PNG (the output) to be.
*) LP_LodePNGColorMode info_raw: here you say what color type of the raw image (the input) has
*) LP_LodePNGEncoderSettings encoder: you can specify a few settings for the encoder to use
...
The encoder will not always exactly match the LP_LodePNGInfo struct you give,
...
*) add_id: add text chunk "Encoder: LP_LodePNG <version>" to the image.
...
An important thing to note about LP_LodePNG, is that the color type of the PNG, and
the color type of the raw image, are completely independent. By default, when
you decode a PNG, you get the result as a raw image in the color type you want,
no matter whether the PNG was encoded with a palette, grayscale or RGBA color.
And if you encode an image, by default LP_LodePNG will automatically choose the PNG
color type that gives good compression based on the values of colors and amount
of colors in the image. It can be configured to let you control it instead as
well, though.

To be able to do this, LP_LodePNG does conversions from one color mode to another.
...
LP_LodePNG automatically picks the best color model for the output PNG, and expects
...
decoding to have another color type, a conversion is done by LP_LodePNG.
...
you need to set the color type and bit depth you want in the LP_LodePNGColorMode,
or the parameters colortype and bitdepth of the simple decoding function.

If, when encoding, you use another color type than the default in the raw input
image, you need to specify its color type and bit depth in the LP_LodePNGColorMode
of the raw image, or use the parameters colortype and bitdepth of the simple
encoding function.

If, when encoding, you don't want LP_LodePNG to choose the output PNG color type
but control it yourself, you need to set auto_convert in the encoder settings
to false, and specify the color type you want in the LP_LodePNGInfo of the
encoder (including palette: it can generate a palette if auto_convert is true,
otherwise not).

If the input and output color type differ (whether user chosen or auto chosen),
LP_LodePNG will do a color conversion, which follows the rules below, and may
sometimes result in an error.

To avoid some confusion:
-the decoder converts from PNG to raw image
-the encoder converts from raw image to PNG
-the colortype and bitdepth in LP_LodePNGColorMode info_raw, are those of the raw image
-the colortype and bitdepth in the color field of LP_LodePNGInfo info_png, are those of the PNG
-when encoding, the color type in LP_LodePNGInfo is ignored if auto_convert
 is enabled, it is automatically generated instead
-when decoding, the color type in LP_LodePNGInfo is set by the decoder to that of the original
 PNG image, but it can be ignored since the raw image has the color type you requested instead
-if the color type of the LP_LodePNGColorMode and PNG image aren't the same, a conversion
 between the color types is done if the color types are supported. If it is not
 supported, an error is returned. If the types are the same, no conversion is done.
-even though some conversions aren't supported, LP_LodePNG supports loading PNGs from any
 colortype and saving PNGs to any colortype, sometimes it just requires preparing
 the raw image correctly before encoding.
-both encoder and decoder use the same color converter.

The function LP_lodepng_convert does the color conversion. It is available in the
...
raw color mode and LP_LodePNGInfo the same color mode, and setting auto_convert to
false.
...
scanline starts at a fresh byte. But that is NOT true for the LP_LodePNG raw input and output.
...
LP_LodePNG uses unsigned char arrays for 16-bit per channel colors too, just like
for any other color format. The 16-bit values are stored in big endian (most
significant byte first) in these arrays. This is the opposite order of the
little endian used by x86 CPU's.

LP_LodePNG always uses big endian because the PNG file format does so internally.
Conversions to other formats than PNG uses internally are not supported by
LP_LodePNG on purpose, there are myriads of formats, including endianness of 16-bit
colors, the order in which you store R, G, B and A, and so on. Supporting and
converting to/from all that is outside the scope of LP_LodePNG.

This may mean that, depending on your use case, you may want to convert the big
endian output of LP_LodePNG to little endian with a for loop. This is certainly not
...
All functions in LP_LodePNG that return an error code, return 0 if everything went
OK, or a non-zero code if there was an error.

The meaning of the LP_LodePNG error values can be retrieved with the function
LP_lodepng_error_text: given the numerical error code, it returns a description
of the error in English as a string.

Check the implementation of LP_lodepng_error_text to see the meaning of each code.
...
If you want to add extra chunks to a PNG you encode, or use LP_LodePNG for a PNG
editor that should follow the rules about handling of unknown chunks, or if your
program is able to read other types of chunks than the ones handled by LP_LodePNG,
then that's possible with the chunk functions of LP_LodePNG.
...
When using LP_lodepng_chunk_next, make sure the returned value is within the
allocated memory.
...
unsigned char LP_lodepng_chunk_critical(const unsigned char* chunk):
unsigned char LP_lodepng_chunk_private(const unsigned char* chunk):
unsigned char LP_lodepng_chunk_safetocopy(const unsigned char* chunk):
...
unsigned char* LP_lodepng_chunk_next(unsigned char* chunk):
const unsigned char* LP_lodepng_chunk_next_const(const unsigned char* chunk):
...
unsigned LP_lodepng_chunk_append(unsigned char** out, size_t* outsize, const unsigned char* chunk):
unsigned LP_lodepng_chunk_create(unsigned char** out, size_t* outsize, unsigned length,
                              const char* type, const unsigned char* data):
...
The LP_LodePNGInfo struct contains fields with the unknown chunk in it. It has 3
...
If you need it to add a particular chunk that isn't known by LP_LodePNG, you can
use LP_lodepng_chunk_append or LP_lodepng_chunk_create to the chunk data in
info_png.unknown_chunks_data[x].

Chunks that are known by LP_LodePNG should not be added in that way. E.g. to make
LP_LodePNG add a bKGD chunk, set background_defined to true and add the correct
...
LP_LodePNG. For the C++ version, only the standard C++ library is needed on top.
...
Make sure that LP_LodePNG is compiled with the same compiler of the same version
and with the same settings as the rest of the program, or the interfaces with
std::vectors and std::strings in C++ can be incompatible.

CHAR_BITS must be 8 or higher, because LP_LodePNG uses unsigned chars for octets.

*) gcc and g++

LP_LodePNG is developed in gcc so this compiler is natively supported. It gives no
...
LP_LodePNG should be warning-free with warning level W4. Two warnings were disabled
...
This is not standard C++ and will not be added to the stock LP_LodePNG. You can
...
LP_LodePNG has been reported to work both with gcc and LLVM for Macintosh, both for
C and C++.
...
This decoder example shows the most basic usage of LP_LodePNG. More complex
examples can be found on the LP_LodePNG website.
...
int main(int argc, char *argv[]) {
  ...
  if(error) std::cout << "decoder error " << error << ": " << LP_lodepng_error_text(error) << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  error = LP_lodepng_decode32_file(&image, &width, &height, filename);
...
A quick reference of some settings to set on the LP_LodePNGState
...
state.info_raw....: more color settings, see struct LP_LodePNGColorMode
state.info_png....: no settings for decoder but ouput, see struct LP_LodePNGInfo
...
state.encoder.add_id: add LP_LodePNG identifier and version as a text chunk
...
state.info_png.color....: more color settings, see struct LP_LodePNGColorMode
state.info_png....: more PNG related settings, see struct LP_LodePNGInfo


12. changes
-----------

The version number of LP_LodePNG is the date of the change given in the format
yyyymmdd.
...
*) 12 jan 2020: (!) added 'end' argument to LP_lodepng_chunk_next to allow correct
   overflow checks.
...
*) 15 jun 2019: (!) auto_choose_color API changed (for bugfix: don't use palette
   if gray ICC profile) and non-ICC LP_LodePNGColorProfile renamed to
   LP_LodePNGColorStats.
...
*) 11 mar 2013: (!) Bugfix with custom free. Changed from "my" to "LP_lodepng_"
...
*) 13 nov 2010: added LP_LodePNG_InfoColor_hasPaletteAlpha and
    LP_LodePNG_InfoColor_canHaveAlpha functions for convenience.
*) 7 nov 2010: added LP_LodePNG_error_text function to get error code description.
...
*) 20 jan 2008: support for unknown chunks allowing using LP_LodePNG for an editor.
*) 18 jan 2008: support for tIME and pHYs chunks added to encoder and decoder.
*) 17 jan 2008: ability to encode and decode compressed zTXt chunks added
    Also various fixes, such as in the deflate and the padding bits code.
*) 13 jan 2008: Added ability to encode Adam7-interlaced images. Improved
    filtering code of encoder.
*) 07 jan 2008: (!) changed LP_LodePNG to use ISO C90 instead of C++. A
    C++ wrapper around this provides an interface almost identical to before.
    Having LP_LodePNG be pure ISO C90 makes it more portable. The C and C++ code
...
*) 19 may 2007: minor fixes, some code cleaning, new error added (error 69),
    moved some examples from here to LP_lodepng_examples.cpp
...
*) 30 jul 2006: (!) LP_LodePNG_InfoPng , width and height are now retrieved in different
    way. Renamed decodePNG to decodePNGGeneric.
*) 29 jul 2006: (!) Changed the interface: image info is now returned as a
    struct of type LP_LodePNG::LP_LodePNG_Info, instead of a vector, which was a bit clumsy.
...
*) 07 jun 2006: (!) Renamed functions to decodePNG and placed them
    in LP_LodePNG namespace. Changed the order of the parameters. Rewrote the
