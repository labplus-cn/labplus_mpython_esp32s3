...
class Forece():
    def __init__(self, i2c=i2c):
        ...
        if DEVICE_ADDR not in i2c_addr:
            raise ValueError(f"未找到设备，地址: {hex(DEVICE_ADDR)}")
    ...
    def read_sensor_data(self):
        ...
        try:
            ...
            if calculated != 0:
                raise ValueError(f"校验和错误 {hex(calculated)}")
    ...
    def read_firmware_version(self):
        ...
        try:
            ...
            # 格式化成字符串
            return f"{version_numbers[0]}.{version_numbers[1]}.{version_numbers[2]}.{version_numbers[3]}"
            
        except Exception as e:
            print(f"读取版本失败: {str(e)}")
