...
class event_loop():
    ...
    def __init__(self, freq=10, timer_id=default_timer_id, max_scheduled=2, refresh_cb=None, asynchronous=False, exception_sink=None):
        if self.is_running():
    ...
    # 刷新协程，实现定时刷新，如果有用户回调，执行用户回调，通过event与定时协程同步
